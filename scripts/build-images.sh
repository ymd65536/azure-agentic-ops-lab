...
docker build \
  --file "${REPO_ROOT}/src/ScribeService/Dockerfile" \
  --tag "scribe-service:${IMAGE_TAG}" \
  "${REPO_ROOT}"

if command -v k3d >/dev/null 2>&1 && k3d cluster list --output json | grep -q "\"name\":\"${CLUSTER_NAME}\""; then
  k3d image import "incident-api:${IMAGE_TAG}" "ops-console:${IMAGE_TAG}" "scribe-service:${IMAGE_TAG}" --cluster "${CLUSTER_NAME}"
else
